...
CODE ZCOUNT     ( str -- addr len )  \ null terminated strings
...
\ Warnings
  ...
  THROW_MSGS LINK, WARN_DEPRECATEDWORD       , ," is a deprecated word"
...
DEFER KEY          ' K_NOOP1 IS KEY
DEFER KEY?         ' K_NOOP1 IS KEY?
DEFER ACCEPT       ' K_NOOP0 IS ACCEPT
DEFER PUSHKEY      ' DROP    IS PUSHKEY
DEFER "PUSHKEYS    ' 2DROP   IS "PUSHKEYS
DEFER SHIFTMASK    ' K_NOOP1 IS SHIFTMASK

DEFER CLS          ' NOOP    IS CLS
DEFER EMIT         ' DROP    IS EMIT
DEFER TYPE         ' 2DROP   IS TYPE
DEFER CR           ' NOOP    IS CR
DEFER ?CR          ' DROP    IS ?CR
DEFER CONSOLE      ' NOOP    IS CONSOLE
DEFER GOTOXY       ' 2DROP   IS GOTOXY
DEFER GETXY        ' K_NOOP2 IS GETXY
...
DEFER FGBG!        ' 2DROP   IS FGBG!
DEFER FG@          ' K_NOOP1 IS FG@
DEFER BG@          ' K_NOOP1 IS BG@
...
DEFER GETROWOFF    ' K_NOOP1 IS GETROWOFF
...
1 PROC ExitProcess
: k_BYE         ( -- )          \ Exit Forth
                _conHndl Call IsWindow
                IF _conHndl call DestroyWindow drop THEN \ close console window
...
: LOWER         ( addr len -- )                  \ translate to uppercase
...
CODE PARSE-WORD ( "<spaces>name" -- c-addr u )
                push    ebx
...
128 constant BFA_IMMEDIATE
 64 constant BFA_DEPRECATED
 32 constant BFA_UNUSED3
 16 constant BFA_UNUSED2
  8 constant BFA_UNUSED1
  4 constant BFA_OFA_PRESENT
  2 constant BFA_FFA_PRESENT
  1 constant BFA_VFA_PRESENT
...
\ problem with LINKFILE completly but 255 char's are more close to 260 than 63.
...
\ to the specific data area; no save is made of the current DP, so
\ it has to be reset back explicitly.
\
...
: IN-APPLICATION ( -- ) ADP TO DP ;   \ set the correct pointer
: IN-SYSTEM      ( -- ) SDP TO DP ;
: IN-CODE        ( -- ) CDP TO DP ;

: >DP           ( dp -- ) R>DROP DP R> 2>R TO DP ; \ save the current DP, set new

: >APPLICATION  ( -- ) ADP >DP ;   \ select app dict, save prev dict
: >SYSTEM       ( -- ) SDP >DP ;   \ select sys dict, save prev dict
: >CODE         ( -- ) CDP >DP ;   \ select code dict, save prev dict

: DP>           ( -- ) 2R> >R TO DP ; \ back to previous DP

' DP> ALIAS SYSTEM>
' DP> ALIAS APPLICATION>
' DP> ALIAS CODE>
...
\ : IN-CODE?      ( -- f ) DP CDP = ;       \ if the DP is set to CDP
...
: DEPRECATED    ( -- )  \ mark the last header create as a deprecated word
...
CODE ADD-LINK   ( addr list -- )                  \ add a link to the front of a list
                pop    eax                        \ fetch addr
...
CODE APPEND-LINK ( addr list -- )                 \ add a link to the end of a list
...
: ",            ( a1 n1 -- )               \ compile a1,n1 at here
                HERE OVER C, OVER ALLOT 1+ SWAP CMOVE
                ;

: ,"            ( -<string">- )            \ compile string" at here
                HERE [CHAR] " PARSE ", 0 C, ALIGN COUNT \N->CRLF
                ;

: Z",           ( a1 n1 -- )               \ compile a1,n1 at here
                HERE OVER ALLOT SWAP CMOVE
                ;

: Z,"           ( -<string">- )            \ compile string" at here
...
CODE +NULL      ( a1 -- )       \ append a NULL just beyond the counted chars
	    ...
	    2dup UPPER                      \ bad; should really copy
            WARNING @ IF
              2DUP CURRENT @ (SEARCH-SELF) IF
                DROP DUP-WARNING?
                IF WARN_NOTUNIQUE WARNMSG THEN
              THEN
            THEN
            ("HEADER)
            ;
...
\ Useage is  ' OLD ALIAS NEW See also SYNONYM for an immediate version
: ALIAS         ( xt -<name>- ) \ make another 'name' for 'xt'
                HEADER CFAPTR! ;

: SYNONYM       ( -<newname> <oldname>- )
\ *p Creates an alias of a word that will be immediate if the original word was
...
: +TO           ( n -<value_name>- )  \ Add to a value as in
                                      \ 10 VALUE X  or  20 +TO X  sets X to 30
                [ 2 CELLS ] LITERAL
                TOCOMPEXEC ; IMMEDIATE
...
: noop-chain-add ( chain_address -- addr )      \ add chain item,
                ...
                begin   dup @
                while   @
                repeat  here swap ! 0 , here ['] noop compile, ;
...
: LINKFILE      ( a1 -- ) \ link name a1 as current file IF LOADING ONLY !!
                ...
                IF
                  ...
                  then r> ?UPPERCASE count 2dup \ uppercase if caps on
...
0 VALUE &PROGNAM                                \ pointer to program name buffer

maxstring 6 * tiblen + equ buffs-len
